...
using DynamicGrids: grid2image, @Image, @Graphic, @Output, 
    ...
    tspan, setfps!, frames, isshowable, Red, Green, Blue
...
@testset "ColorProcessor" begin
    proc = ColorProcessor(zerocolor=RGB24(1.0,0.0,0.0))
    output = TestImageOutput(init; processor=proc, minval=0.0, maxval=10.0, store=true)
    @test minval(output) === 0.0
    @test maxval(output) === 10.0
    @test processor(output) == ColorProcessor(zerocolor=RGB24(1.0,0.0,0.0))
...
@testset "ThreeColorProcessor" begin
    multiinit = (a = [5.0 5.0 4.0 4.0], b = [0.1 0.2 0.0 0.0], c = [5.0 5.0 10.0 5.0])
    proc = ThreeColorProcessor(colors=(Red(), Green(), Blue()), zerocolor=RGB24(0.9))
    output = TestImageOutput(multiinit; processor=proc, minval=(4, 0, 5), maxval=(6, 1, 10), store=true)
    @test minval(output) === (4, 0, 5)
    @test maxval(output) === (6, 1, 10)
    @test processor(output) === proc

    # Test image is combined from red and green overlays
    @test grid2image(processor(output), output, Ruleset(), multiinit, 1) ==
        [RGB24(0.5, 0.1, 0.0) RGB24(0.5, 0.2, 0.0) RGB24(0.0, 0.0, 1.0) RGB24(0.9, 0.9, 0.9)]
end
